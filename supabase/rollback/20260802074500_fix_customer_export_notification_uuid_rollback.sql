...
BEGIN
  ...
  IF old_token_count <> 0 OR new_token_count <> 6 THEN
    RAISE EXCEPTION
      'organization_customer_snapshot_notification_rollback_drift:%:%',
      old_token_count, new_token_count;
  END IF;

  restored_sql := replace(function_sql, new_token, old_token);
  EXECUTE restored_sql;
  ...
  THEN
    RAISE EXCEPTION 'organization_customer_snapshot_notification_rollback_failed';
  END IF;
END
$rollback$;
