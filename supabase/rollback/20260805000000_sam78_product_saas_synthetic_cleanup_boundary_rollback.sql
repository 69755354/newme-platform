...
BEGIN
  IF COALESCE(current_setting('newme.environment', true), '')
    ...
    RAISE EXCEPTION 'sam78_product_saas_cleanup_rollback_requires_staging_or_test';
  END IF;
END
$$;
...
REVOKE ALL ON FUNCTION public.v4_guard_platform_action_approval_update()
  FROM PUBLIC, anon, authenticated;
DROP FUNCTION public.product_saas_is_synthetic_audit_event(uuid);
DROP FUNCTION public.product_saas_is_synthetic_audit_log(uuid);
DROP FUNCTION public.product_saas_is_synthetic_exit_approval(uuid);
DROP FUNCTION public.product_saas_is_synthetic_organization(uuid);
