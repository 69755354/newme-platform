...
>
> **仍需生产授权的动作（本轮一律未执行）**：应用 9 个待应用迁移（`20260813000000` 另需对 `auth.users` 的 SELECT）；封禁 `dev@newme.ae` 的 Auth 身份并吊销会话；凭证轮换；仓库转 private；git 历史清除；origin 防火墙限定 Cloudflare 段；`/etc/newme/migration-db.url` 落地。
>
