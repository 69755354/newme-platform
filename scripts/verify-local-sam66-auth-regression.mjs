 ...
 *
 * This intentionally has no dotenv loader: operators must supply local
 * Supabase keys through their process environment. Credentials are never
 * written to the repository, artifacts, or report output.
 */
...
async function api(token, path, { method = "GET", body } = {}) {
  ...
  if (response.status >= 500) {
    fail(`${method} ${path} returned HTTP ${response.status}: ${safeText(JSON.stringify(payload))}`);
  }
...
function expectStatus(label, response, status, requiredText) {
  ...
  if (requiredText) {
    assert.match(JSON.stringify(response.payload), requiredText, `${label} response body mismatch`);
  }
}
...
function exactMarker(user, marker) {
  ...
  return meta.fixture_scope === "sam66-local-auth-regression"
    && meta.fixture_kind === "auth-gate"
    && meta.run_id === marker;
}
...
async function createActor(admin, marker, actor, users) {
  ...
  const { data, error } = await admin.auth.admin.createUser({
    ...
    email_confirm: true,
    app_metadata: {
      fixture_scope: "sam66-local-auth-regression",
      fixture_kind: "auth-gate",
      run_id: marker,
      actor,
    },
    user_metadata: { full_name: `[SAM-66 ${marker}] ${actor}` },
  });
  if (error || !data.user || !exactMarker(data.user, marker)) {
    fail("local auth user creation did not retain its exact marker");
  }
  const { error: profileError } = await admin.from("profiles").update({
    role: actor === "sales-other" ? "sales" : actor,
  ...
  if (signInError || !signedIn.session || signedIn.user?.id !== data.user.id) {
    fail("password authentication did not produce the created local user");
  }
  const { data: refreshed, error: refreshError } = await client.auth.refreshSession({
    refresh_token: signedIn.session.refresh_token,
  });
  if (refreshError || !refreshed.session || refreshed.user?.id !== data.user.id) {
    fail("refresh authentication did not retain the local user");
  }
...
async function createLeadFor(admin, marker, user, suffix, leadIds) {
  ...
  if (error || !data?.id || data.assigned_to !== user.id || data.stage !== "new") {
    fail("marked lead creation/readback failed");
  }
...
async function getContactId(response) {
  const candidate = response.payload?.contact?.contact_id
    ?? response.payload?.contact?.id
    ?? response.payload?.contact_id;
...
async function runApiFlows(marker, users, leadIds, checks) {
  for (const actor of ACTORS) {
    ...
    assert.equal(
      me.payload?.role,
      actor === "sales-other" ? "sales" : actor,
      `${actor} profile role readback mismatch`,
    );
  }
  ...
  const primary = await createLeadFor(admin, marker, users.get("sales"), "primary", leadIds);
  const crossOwner = await createLeadFor(admin, marker, users.get("sales-other"), "cross-owner", leadIds);
  const lossLead = await createLeadFor(admin, marker, users.get("sales"), "lost", leadIds);
  const salesList = await api(users.get("sales").token, "/api/leads/list");
  expectStatus("sales lead read", salesList, 200);
  assert.ok(
    salesList.payload?.leads?.some((lead) => lead.id === primary.id),
    "sales lead list omitted its owned marked lead",
  );
  assert.ok(
    !salesList.payload?.leads?.some((lead) => lead.id === crossOwner.id),
    "sales lead list exposed a cross-owner marked lead",
  );

  const crossOwnerDenied = await api(users.get("sales").token, `/api/leads/${crossOwner.id}/notes`, {
    method: "POST",
    body: { note: `[SAM-66 ${marker}] forbidden`, idempotencyKey: randomUUID() },
  });
  ...
  const contactOne = await api(users.get("sales").token, `/api/leads/${primary.id}/contacts`, {
    ...
    body: {
      ...
      contact_result: `[SAM-66 ${marker}] reached`,
      summary: `[SAM-66 ${marker}] first contact`,
    },
  });
  expectStatus("primary contact create", contactOne, 200, /success/i);
  await getContactId(contactOne);
  const quality = await api(users.get("sales").token, `/api/leads/${primary.id}/quality`, {
    method: "POST",
    body: { quality: "good" },
  });
  ...
  const firstContact = await api(users.get("sales").token, `/api/leads/${primary.id}/milestone`, {
    method: "POST",
    body: { milestoneKey: "first_contact", notes: `[SAM-66 ${marker}] explicit confirmation` },
  });
  ...
  const contactTwo = await api(users.get("sales").token, `/api/leads/${primary.id}/contacts`, {
    ...
    body: {
      ...
      contact_time: new Date(Date.now() - 3_000).toISOString(),
      contact_result: `[SAM-66 ${marker}] follow-up`,
      summary: `[SAM-66 ${marker}] editable`,
    },
  });
  ...
  const edited = await api(users.get("sales").token, `/api/leads/${primary.id}/contacts/${contactId}`, {
    ...
    body: {
      contact_method: "whatsapp",
      contact_time: new Date(Date.now() - 2_000).toISOString(),
      contact_result: `[SAM-66 ${marker}] edited`,
      summary: `[SAM-66 ${marker}] edited timeline`,
    },
  });
  ...
  const deleted = await api(
    users.get("sales").token,
    `/api/leads/${primary.id}/contacts/${contactId}`,
    { method: "DELETE" },
  );
  ...
  assert.doesNotMatch(
    JSON.stringify(timelineAfterDelete.payload),
    new RegExp(contactId, "i"),
    "deleted contact remained in timeline readback",
  );
  ...
  for (const [lead, terminal] of [[primary, "won"], [lossLead, "lost"]]) {
    if (lead.id === lossLead.id) {
      const contact = await api(users.get("sales").token, `/api/leads/${lead.id}/contacts`, {
        ...
        body: {
          contact_method: "phone",
          contact_time: new Date(Date.now() - 4_000).toISOString(),
          contact_result: `[SAM-66 ${marker}] loss contact`,
          summary: `[SAM-66 ${marker}] loss`,
        },
      });
      expectStatus("lost lead contact", contact, 200, /success/i);
      const qualityLoss = await api(users.get("sales").token, `/api/leads/${lead.id}/quality`, {
        method: "POST",
        body: { quality: "normal" },
      });
    ...
    const transition = await api(users.get("sales").token, `/api/leads/${lead.id}/stage`, {
      ...
      body: {
        stage: terminal,
        note: `[SAM-66 ${marker}] ${terminal}`,
        idempotencyKey: randomUUID(),
      },
    });
    expectStatus(`${terminal} stage transition`, transition, 200, new RegExp(terminal));
  }
  const readback = await admin.from("leads")
    .select("id,stage,final_status")
    .in("id", [primary.id, lossLead.id]);
  if (readback.error || readback.data?.length !== 2) fail("terminal lead readback failed");
  for (const row of readback.data) {
    assert.equal(row.stage, row.final_status, "terminal stage and status diverged");
  }
  checks.push(
    "auth-refresh",
    "dashboard",
...
async function runUi(users) {
  ...
  try {
    ...
    if (/executable|browserType\.launch|headless_shell|chromium/i.test(message)) return false;
    throw error;
  }
  try {
    for (const actor of ["boss", "admin", "operator", "sales"]) {
      ...
      await page.waitForURL(
        (url) => url.origin === new URL(BASE_URL).origin && url.pathname !== "/login",
        { timeout: 20_000 },
      );
      const me = await page.evaluate(async () => ({
        status: (await fetch("/api/auth/me", { cache: "no-store" })).status,
      }));
...
async function cleanup(marker, leadIds) {
  const admin = serviceClient();
  const capture = async (label, operation) => {
    const { error } = await operation;
    if (error) {
      fail(`marker-scoped cleanup failed for ${label} (${error.code ?? error.status ?? "unknown"})`);
    }
  };
  ...
  for (const user of matchingUsers) {
    await capture(
      ...
      admin.from("user_session_daily").delete().eq("user_id", user.id),
    );
    await capture(
      "audit_logs.actor_id",
      admin.from("audit_logs").delete().eq("actor_id", user.id),
    );
    const { error } = await admin.auth.admin.deleteUser(user.id, false);
    if (error) {
      fail(`marked auth identity cleanup failed (status ${error.status ?? "unknown"}: ${safeText(error.message)})`);
    }
  }
  ...
  const residues = {
    ...
    profiles: (await admin.from("profiles")
      .select("id", { count: "exact", head: true })
      .like("full_name", `${markerText}%`)).count ?? -1,
    leads: (await admin.from("leads")
      .select("id", { count: "exact", head: true })
      .like("customer_name", `${markerText}%`)).count ?? -1,
    activities: (await admin.from("activities")
      .select("id", { count: "exact", head: true })
      .like("content", `${markerText}%`)).count ?? -1,
    tasks: (await admin.from("tasks")
      .select("id", { count: "exact", head: true })
      .like("title", `${markerText}%`)).count ?? -1,
    business_events: (await admin.from("business_events")
      .select("id", { count: "exact", head: true })
      .like("description", `${markerText}%`)).count ?? -1,
    notifications: leadIds.length === 0
      ? 0
      : (await admin.from("notifications")
        .select("id", { count: "exact", head: true })
        .in("related_id", leadIds)).count ?? -1,
  };
  if (Object.values(residues).some((count) => count !== 0)) {
    fail("marker residue remains after cleanup");
  }
}
...
async function recoverInterruptedLocalRuns() {
  ...
  if (markers.some((marker) => typeof marker !== "string" || !marker)) {
    fail("refusing recovery for a malformed local SAM-66 marker");
  }
  for (const marker of markers) {
    const users = scoped.filter((user) => exactMarker(user, marker));
    if (users.length !== ACTORS.length) {
      fail("refusing recovery for an incomplete local SAM-66 actor set");
    }
    const { data: leads, error } = await admin.from("leads")
      .select("id")
      .like("customer_name", `[SAM-66 ${marker}]%`);
    if (error) fail("cannot discover marked leads for interrupted local SAM-66 cleanup");
    await cleanup(marker, (leads ?? []).map((lead) => lead.id));
  }
}
...
async function runOnce(index) {
  ...
  try {
    const admin = serviceClient();
    for (const actor of ACTORS) {
      await createActor(admin, marker, actor, users);
    }
    await runApiFlows(marker, users, leadIds, checks);
    const uiCovered = await runUi(users);
    REPORT.ui = uiCovered ? "covered" : "not-covered-browser-unavailable";
    if (!uiCovered) {
      fail("browser coverage unavailable; install Chromium before running the full SAM-66 gate");
    }
  } catch (error) {
    primaryError = error;
  } finally {
    await cleanup(marker, leadIds);
  }
...
main().catch((error) => {
  console.error(JSON.stringify({
    ...
    error: error instanceof Error ? safeText(error.message) : "SAM66_FAILED",
  }));
  process.exitCode = 1;
});
