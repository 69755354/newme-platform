...
readonly SAM78_MIGRATION_060900="supabase/migrations/20260806090000_sam78_product_saas_billable_seat_cleanup_boundary.sql"
readonly SAM78_MIGRATION_061000="supabase/migrations/20260806100000_sam78_product_saas_inactive_exit_approval_cleanup_boundary.sql"
readonly SAM78_MIGRATION_061100="supabase/migrations/20260806110000_sam78_product_saas_exit_staff_ref_cleanup_boundary.sql"
readonly SAM78_MIGRATION_061200="supabase/migrations/20260806120000_sam78_organizations_selected_context_read_boundary.sql"
...
readonly SAM78_ROLLBACK_060900="supabase/rollback/20260806090000_sam78_product_saas_billable_seat_cleanup_boundary_rollback.sql"
readonly SAM78_ROLLBACK_061000="supabase/rollback/20260806100000_sam78_product_saas_inactive_exit_approval_cleanup_boundary_rollback.sql"
readonly SAM78_ROLLBACK_061100="supabase/rollback/20260806110000_sam78_product_saas_exit_staff_ref_cleanup_boundary_rollback.sql"
readonly SAM78_ROLLBACK_061200="supabase/rollback/20260806120000_sam78_organizations_selected_context_read_boundary_rollback.sql"
...
require_sam78_apply_evidence() {
  ...
  /usr/bin/node -e '
    ...
    const versions = [
      ...
      "20260805190000", "20260806000000", "20260806010000", "20260806020000", "20260806030000", "20260806040000", "20260806050000", "20260806060000", "20260806070000", "20260806080000", "20260806090000", "20260806100000", "20260806110000", "20260806120000",
    ];
...
run_sam78_database_action() {
  ...
  migration_060900="$run_dir/20260806090000.sql"
  migration_061000="$run_dir/20260806100000.sql"
  migration_061100="$run_dir/20260806110000.sql"
  migration_061200="$run_dir/20260806120000.sql"
  ...
  rollback_060900="$run_dir/20260806090000.rollback.sql"
  rollback_061000="$run_dir/20260806100000.rollback.sql"
  rollback_061100="$run_dir/20260806110000.rollback.sql"
  rollback_061200="$run_dir/20260806120000.rollback.sql"
  ...
  copy_commit_blob "$SHA" "$SAM78_MIGRATION_060900" "$migration_060900"
  copy_commit_blob "$SHA" "$SAM78_MIGRATION_061000" "$migration_061000"
  copy_commit_blob "$SHA" "$SAM78_MIGRATION_061100" "$migration_061100"
  copy_commit_blob "$SHA" "$SAM78_MIGRATION_061200" "$migration_061200"
  ...
  copy_commit_blob "$SHA" "$SAM78_ROLLBACK_060900" "$rollback_060900"
  copy_commit_blob "$SHA" "$SAM78_ROLLBACK_061000" "$rollback_061000"
  copy_commit_blob "$SHA" "$SAM78_ROLLBACK_061100" "$rollback_061100"
  copy_commit_blob "$SHA" "$SAM78_ROLLBACK_061200" "$rollback_061200"
  ...
  migration_060900_blob="$(git --git-dir="$REPOSITORY" rev-parse "$SHA:$SAM78_MIGRATION_060900")"
  migration_061000_blob="$(git --git-dir="$REPOSITORY" rev-parse "$SHA:$SAM78_MIGRATION_061000")"
  migration_061100_blob="$(git --git-dir="$REPOSITORY" rev-parse "$SHA:$SAM78_MIGRATION_061100")"
  migration_061200_blob="$(git --git-dir="$REPOSITORY" rev-parse "$SHA:$SAM78_MIGRATION_061200")"
  ...
  rollback_060900_blob="$(git --git-dir="$REPOSITORY" rev-parse "$SHA:$SAM78_ROLLBACK_060900")"
  rollback_061000_blob="$(git --git-dir="$REPOSITORY" rev-parse "$SHA:$SAM78_ROLLBACK_061000")"
  rollback_061100_blob="$(git --git-dir="$REPOSITORY" rev-parse "$SHA:$SAM78_ROLLBACK_061100")"
  rollback_061200_blob="$(git --git-dir="$REPOSITORY" rev-parse "$SHA:$SAM78_ROLLBACK_061200")"
  ...
  chmod 0400 "$verify" "$history_manifest" "$migration_031000" "$migration_143000" \
    "$migration_041530" "$migration_041657" "$migration_041853" "$migration_041930" "$migration_050000" "$migration_050100" "$migration_050200" "$migration_051200" "$migration_051300" "$migration_051900" "$migration_060000" "$migration_060100" "$migration_060200" "$migration_060300" "$migration_060400" "$migration_060500" "$migration_060600" "$migration_060700" "$migration_060800" "$migration_060900" "$migration_061000" "$migration_061100" "$migration_061200" "$rollback_031000" "$rollback_143000" \
  ...
  /usr/bin/env -i \
    ...
    SAM78_MIGRATION_060900_PATH="$migration_060900" \
    SAM78_MIGRATION_060900_BLOB="$migration_060900_blob" \
    SAM78_MIGRATION_061000_PATH="$migration_061000" \
    SAM78_MIGRATION_061000_BLOB="$migration_061000_blob" \
    SAM78_MIGRATION_061100_PATH="$migration_061100" \
    SAM78_MIGRATION_061100_BLOB="$migration_061100_blob" \
    SAM78_MIGRATION_061200_PATH="$migration_061200" \
    SAM78_MIGRATION_061200_BLOB="$migration_061200_blob" \
    ...
    SAM78_ROLLBACK_060900_PATH="$rollback_060900" \
    SAM78_ROLLBACK_060900_BLOB="$rollback_060900_blob" \
    SAM78_ROLLBACK_061000_PATH="$rollback_061000" \
    SAM78_ROLLBACK_061000_BLOB="$rollback_061000_blob" \
    SAM78_ROLLBACK_061100_PATH="$rollback_061100" \
    SAM78_ROLLBACK_061100_BLOB="$rollback_061100_blob" \
...
NODE
  ...
  /usr/bin/node -e '
    ...
    const versions = [
      "20260803100000", "20260803143000", "20260804153000",
      "20260804165734", "20260804185311", "20260804193000", "20260805000000",
      "20260805010000", "20260805020000", "20260805120000", "20260805130000", "20260805190000", "20260806000000", "20260806010000", "20260806020000", "20260806030000", "20260806040000", "20260806050000", "20260806060000", "20260806070000", "20260806080000", "20260806090000", "20260806100000", "20260806110000", "20260806120000",
    ];
