...
BEGIN
  IF current_setting('newme.environment', true) NOT IN ('staging', 'test') THEN
    RAISE EXCEPTION 'sam78_product_saas_inactive_exit_approval_cleanup_rollback_requires_staging_or_test';
  END IF;
END
$$;
    ...
    WHERE approval.id = p_approval_id
      ...
      AND (
        ...
        OR (approval.status = 'approved'
          AND approver.staff_ref = 'EXIT-' || left(substring(organization.slug FROM 14), 8) || '-APP'
