-- =============================================================================
-- Fix on_lead_won() trigger: use valid event types for business_events and activities
-- + fix total_contract_amount for new customers (ELSE branch)
-- 2026-06-03
-- =============================================================================

-- The original function used 'lead_won_automation' (not in chk_event_type constraint)
-- and 'system_automation' (not in activities_type_check constraint), causing
-- transaction rollback on every lead_won.
-- Also, the ELSE branch (new customer creation) omitted total_contract_amount.
...
DECLARE
  ...
  v_due_days integer[];
BEGIN
  -- Only trigger when stage changes TO 'won' (not from 'won' to something else)
