...
BEGIN
  IF COALESCE(current_setting('newme.environment', true), '')
    ...
    RAISE EXCEPTION 'sam78_product_saas_inactive_audit_cleanup_rollback_requires_staging_or_test';
  END IF;
END
$$;
...
AS $$
  SELECT EXISTS (
    ...
    JOIN public.organizations AS organization ON organization.id = audit_event.organization_id
    JOIN public.profiles AS actor_profile ON actor_profile.id = audit_event.actor_user_id
    ...
    WHERE audit_event.id = p_audit_event_id
      ...
      AND (
        (audit_event.action = 'lead.import' AND audit_event.target_type = 'import_batch'
        ...
        OR (audit_event.action = 'organization_membership_role_created'
          ...
          AND EXISTS (SELECT 1 FROM public.memberships AS target_membership
        ...
        OR (audit_event.action IN ('organization.exit_prepared', 'organization.exit_completed')
          AND audit_event.target_type = 'organization'
          AND audit_event.target_id = organization.id::text
          AND EXISTS (SELECT 1 FROM public.platform_action_approvals AS approval
