-- Migration: Fix lead_alerts v2 view + notifications constraint
-- 1. Add missing quotation_sent_date column to leads (used by v2 view)
-- 2. Add followup_reminder variant to notifications constraint (TS code convention)
-- 3. Recreate lead_alerts view with proper column references
-- Date: 2026-06-13
...
-- The TS code uses both followup_reminder (no underscore) and follow_up_overdue (with underscore)
-- Latest migration 20260612000002 uses follow_up_reminder (with underscore).
-- Ensure both variants are accepted.
