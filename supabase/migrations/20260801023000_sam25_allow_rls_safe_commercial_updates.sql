...
BEGIN
  ...
  IF TG_OP = 'UPDATE' THEN
    ...
    END IF;
    -- Parent/organization composite foreign keys preserve the same-organization
    -- invariant without requiring an RLS-visible parent lookup for ordinary
    -- updates performed by finance and other permitted roles.
    expected_organization_id := OLD.organization_id;
  ELSE
    CASE TG_TABLE_NAME
