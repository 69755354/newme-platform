-- Serialize identical mutation keys before checking their replay records.
-- This closes the race where concurrent requests both missed the initial
-- lookup and the second request failed on the request-table unique key.
...
BEGIN
  ...
  PERFORM pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended(
      v_actor_id::text || ':lead_delete:' || p_idempotency_key::text,
      0
    )
  );
...
BEGIN
  ...
  PERFORM pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended(
      v_actor_id::text || ':lead_reassignment:' || p_idempotency_key::text,
      0
    )
  );
...
BEGIN
  ...
  PERFORM pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended(
      v_actor_id::text || ':lead_note:' || p_idempotency_key::text,
      0
    )
  );
...
BEGIN
  ...
  PERFORM pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended(
      v_actor_id::text || ':lead_contact:' || p_idempotency_key::text,
      0
    )
  );
...
-- Transfer history is audit evidence. Authenticated callers may read it through
-- RLS, but must not bypass reassign_lead_atomic to forge, edit, or delete rows.
