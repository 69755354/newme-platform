...
if [ "$HTTP_CODE" = "000" ]; then
  ...
  bash "$ALERT_SCRIPT" "login-probe" failure "health endpoint unavailable" || true
...
if [ "$HTTP_CODE" -ge 500 ]; then
  ...
  bash "$ALERT_SCRIPT" "login-probe" failure "health endpoint returned HTTP $HTTP_CODE" || true
...
while [ $attempt -le $MAX_RETRIES ]; do
  ...
  case "$AUTH_CODE" in
    200|401|307)
      bash "$ALERT_SCRIPT" "login-probe" recovery "login probe recovered" || true
...
bash "$ALERT_SCRIPT" "login-probe" failure "auth probe failed: $last_error" || true
