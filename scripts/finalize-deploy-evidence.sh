...
if migration.get("status") not in {"not_required", "applied_verified", "reentry_verified", "contract_verified"}:
    fail("migration status is not verified")
if migration.get("status") in {"applied_verified", "reentry_verified", "contract_verified"} and not migration.get("ids"):
