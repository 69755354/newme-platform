...
状态：Target；基于已合并 PR #256 的治理基线，不构成运行时实现声明
版本：2026-08-04
最终上游复核：`agent/saas-staging-isolation@858a4ccb51697b4b4499252bfa3c22963381847e`
...
前端只负责呈现、采集意图和反馈结果。授权、租户边界、状态机、金额/库存/合规计算、幂等和审计必须由 #256 定义的平台与垂直服务在确切实现 SHA 上提供；治理文档已合并不等于这些运行时能力已经实现。
...
- 不重写认证、租户、RLS、计费、状态机或事件基础设施；这些是 #256 定义的上游实现责任，只有对应代码、测试和环境证据存在时才可声明已交付。
