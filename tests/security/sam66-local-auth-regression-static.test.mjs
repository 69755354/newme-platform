...
const runner = await readFile(new URL("../../scripts/verify-local-sam66-auth-regression.mjs", import.meta.url), "utf8");
...
test("SAM-66 is loopback-only, marker-scoped, and runs twice", () => {
  ...
  for (const table of ["profiles", "leads", "activities", "tasks", "business_events", "notifications"]) {
    assert.match(runner, new RegExp(`${table}:`));
  }
});

test("SAM-66 exercises real credentials, refresh, multi-role UI, and cross-owner denial", () => {
  for (const actor of ["boss", "admin", "operator", "sales", "sales-other"]) assert.match(runner, new RegExp(`"${actor}"`));
  assert.match(runner, /signInWithPassword/);
  assert.match(runner, /refreshSession/);
  assert.match(runner, /chromium\.launch/);
  assert.match(runner, /playwright", "install", "chromium/);
...
test("SAM-66 never persists credentials or default Playwright artifacts", () => {
