...
> v1.7 — Schema 一致性修正：follow_up_logs 字段名与实际生产对齐，leads扩展字段标记 Deferred，funnel快照模型修正为窄表。ARCHITECTURE_RULES.yaml 已确认为唯一 Schema Authority (rule_011)。
...
#### Migration 004: follow_up_logs（行为日志，不记录分配关系）

> ⚠️ v1.6 修正：以下 schema 已从 DEV_PLAN v1.5 的 `sales_id`/`contact_method`/`content` 模型修正为 `user_id`/`contact_type`/`summary` 模型。原因见 `docs/audit/architecture_baseline_audit.md` 4.1 节。
...
CREATE TABLE follow_up_logs (
  ...
  -- user_id = 操作主体（谁做的），不是 sales_id（客户归属）
  -- 客户归属从 leads.assigned_to 查询
  contact_type TEXT NOT NULL DEFAULT 'phone',
  summary TEXT NOT NULL DEFAULT '',
  result TEXT,
  no_answer BOOLEAN NOT NULL DEFAULT false,
  next_action TEXT,
  -- next_action 有值时，DB trigger 自动在 tasks 表创建待办
  created_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
CREATE INDEX idx_follow_up_logs_lead ON follow_up_logs(lead_id);
CREATE INDEX idx_follow_up_logs_created ON follow_up_logs(created_at DESC);
```

**follow_up_logs 只记录「已经发生了什么」**。有 `next_action` 时 DB trigger 自动在 tasks 表创建未来待办。
...
#### Deferred — Phase B 字段池（待业务需求确认后逐批加入）

> 以下字段在 Phase A 不执行 migration。列为 Deferred 的原因是 MVP 阶段不应预先塞入全部"可能有用"的字段，见 `docs/audit/architecture_baseline_audit.md` 第5节。

| 字段 | 类型 | 目标 Phase | 说明 |
|------|------|-----------|------|
| contact_result | TEXT CHECK | Phase B | 联系结果：interested/not_interested/no_answer |
| not_interested_reason | TEXT CHECK | Phase B | 不感兴趣原因 |
| project_type | TEXT CHECK | Phase B | 项目类型：villa/apartment/developer |
| project_status | TEXT CHECK | Phase B | under_construction/ready/renovation |
| emirate | TEXT | Phase B | 酋长国 |
| area | TEXT | Phase B | 区域 |
| customer_company_type | TEXT | Phase B | 客户公司类型 |
| customer_position | TEXT | Phase B | 客户职位 |
| ac_brand | TEXT | Phase B | 空调品牌需求 |
| smart_requirements | JSONB | Phase B | 智能需求清单 |
| customer_budget | NUMERIC(12,2) | Phase B | 客户预算 |
| expected_sign_date | DATE | Phase B | 预计签约日期 |
| lost_reason | TEXT CHECK | Phase B | 丢单原因 |
...
```sql
-- crm_daily_funnel_snapshot（每天 cron 执行一次）
-- 窄表模型：每行 = 一个milestone的计数
-- 自动适应新milestone，无需改表结构
CREATE TABLE IF NOT EXISTS crm_daily_funnel_snapshot (
  snapshot_date DATE NOT NULL DEFAULT CURRENT_DATE,
  current_milestone TEXT NOT NULL,
  lead_count INTEGER NOT NULL DEFAULT 0,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  PRIMARY KEY (snapshot_date, current_milestone)
);

-- 每日插入：按 current_milestone 分组统计（非累计覆盖率）
INSERT INTO crm_daily_funnel_snapshot (
  snapshot_date, current_milestone, lead_count
)
SELECT
  CURRENT_DATE,
  l.current_milestone,
  COUNT(*)::INTEGER
FROM leads l
WHERE l.current_milestone IS NOT NULL
   OR l.final_status IS NOT NULL
GROUP BY l.current_milestone
UNION ALL
SELECT CURRENT_DATE, 'won', COUNT(*)::INTEGER
FROM leads WHERE final_status = 'won'
UNION ALL
SELECT CURRENT_DATE, 'lost', COUNT(*)::INTEGER
FROM leads WHERE final_status = 'lost';
```

三个月后，老板可以看到真正的漏斗：
```
new=100 → contacted=80 → qualified=60 → drawings=40 → requirements=20
→ solution=15 → quotation=10 → meeting=8 → won=5  |  lost=10
```
每一级是**当前所在层的线索数**，不是累计覆盖。哪一级流失最严重一目了然。

> 若后续宽表查询出现性能瓶颈，可通过物化视图从窄表转换为宽表供看板使用，不替换原表。
