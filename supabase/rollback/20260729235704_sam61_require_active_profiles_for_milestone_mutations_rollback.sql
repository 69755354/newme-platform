-- Controlled rollback for 20260729235704.
-- This removes the inactive-profile guard and therefore requires an explicit
-- non-production environment marker. Function ownership and grants are retained.

DO $guard$
DECLARE
  environment_name text := current_setting('newme.environment', true);
BEGIN
  IF environment_name IS DISTINCT FROM 'staging'
     AND environment_name IS DISTINCT FROM 'test' THEN
    RAISE EXCEPTION USING
      ERRCODE = '42501',
      MESSAGE = 'sam61_active_profile_rollback_requires_staging_or_test';
  END IF;
END;
$guard$;
