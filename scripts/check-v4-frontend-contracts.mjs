...
export function validateStagingArchiveProvenance(root, upstream, dependencies = {}) {
  ...
  for (const record of indexEntries.split('\0').filter(Boolean)) {
    const separator = record.indexOf('\t')
    invariant(separator > 0, 'staging archive index entry is malformed')
    const [mode] = record.slice(0, separator).split(' ')
    if (mode !== '160000') continue
    const gitlinkPath = path.join(root, record.slice(separator + 1))
    if (!fs.existsSync(gitlinkPath)) continue
    const gitlink = fs.lstatSync(gitlinkPath)
