...
export function validateEvidenceDocument(schemaName, value) {
  if (!schemaNames.includes(schemaName) || ["bundle", "sam85Bundle"].includes(schemaName)) fail("unknown_schema", `$.${schemaName}`);
...
export function validateSam85RehearsalBundle(bundle, options = {}) {
  scanSensitiveMaterial(bundle);
  validateSchema(bundle, evidenceSchemas.sam85Bundle);
  if (options.expectedMode && bundle.mode !== options.expectedMode) fail("bundle_mode_mismatch", "$.mode");
  ...
  if (!executed) {
    if (bundle.evidenceState !== "target" || bundle.environmentClass !== "synthetic-local") {
      fail("template_evidence_state_invalid", "$");
    }
  } else {
    if (bundle.environmentClass === "synthetic-local" && options.allowSyntheticEvidence !== true) {
      fail("synthetic_execution_not_evidence", "$.environmentClass");
    }
    if (bundle.environmentClass === "isolated-ephemeral-clone" && bundle.evidenceState !== "verified-current") {
      fail("environment_evidence_state_mismatch", "$.evidenceState");
    }
  }
  exactSet(bundle.linearIds, ["SAM-85"], "linear_id_set_invalid", "$.linearIds");
  ...
  if (executed) {
    ...
    const verifiedAt = timestamp(bundle.destruction.verifiedAt, "destruction_verified_at_invalid", "$.destruction.verifiedAt");
    if (verifiedAt < destroyedAt) fail("destruction_verified_too_early", "$.destruction.verifiedAt");
    if (timestamp(bundle.generatedAt, "bundle_generated_at_invalid", "$.generatedAt") < verifiedAt) {
      fail("bundle_generated_before_evidence_complete", "$.generatedAt");
    }
  }
  return Object.freeze({
    ...
    acceptanceStatus: executed ? "passed" : "not_executed",
