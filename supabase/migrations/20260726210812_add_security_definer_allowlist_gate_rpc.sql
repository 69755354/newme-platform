CREATE OR REPLACE FUNCTION public.security_definer_rpc_allowlist_violations()
RETURNS TABLE (violation text, regprocedure text)
LANGUAGE sql
STABLE
SECURITY INVOKER
...
REVOKE ALL ON FUNCTION public.security_definer_rpc_allowlist_violations()
FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.security_definer_rpc_allowlist_violations()
TO service_role;

COMMENT ON FUNCTION public.security_definer_rpc_allowlist_violations() IS
  'Service-role-only live deployment gate for the authenticated SECURITY DEFINER RPC allowlist.';
