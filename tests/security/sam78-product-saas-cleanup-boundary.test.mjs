...
test("SAM-78 Product/SaaS cleanup remains exact, service-only, and reversible", async () => {
  const [migration, rollback, inactiveCleanupMigration, inactiveCleanupRollback, staffRefCleanupMigration, staffRefCleanupRollback, runner, controller, databaseGate] = await Promise.all([
    ...
    read("supabase/migrations/20260806110000_sam78_product_saas_exit_staff_ref_cleanup_boundary.sql"),
    read("supabase/rollback/20260806110000_sam78_product_saas_exit_staff_ref_cleanup_boundary_rollback.sql"),
  ...
  assert.match(staffRefCleanupMigration, /requester\.staff_ref = 'SAM79-'/);
  assert.match(staffRefCleanupMigration, /approver\.staff_ref = 'SAM79-'/);
  assert.doesNotMatch(staffRefCleanupMigration, /staff_ref = 'EXIT-'/);
  assert.doesNotMatch(staffRefCleanupMigration, /profile\.is_active IS TRUE/);
  assert.match(staffRefCleanupRollback, /staff_ref = 'EXIT-'/);
  assert.doesNotMatch(staffRefCleanupRollback, /profile\.is_active IS TRUE/);
  assert.match(staffRefCleanupMigration, /REVOKE ALL ON FUNCTION public\.product_saas_is_synthetic_exit_approval[\s\S]*FROM PUBLIC, anon, authenticated/);
  assert.match(staffRefCleanupMigration, /GRANT EXECUTE ON FUNCTION public\.product_saas_is_synthetic_exit_approval[\s\S]*TO service_role/);
